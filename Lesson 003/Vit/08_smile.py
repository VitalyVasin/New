...
def smile (x1, y1, color):
    ...
    sd.line(point3, point4, color=color, width=4)
    point5 = sd.get_point(x1+90, y1+10)
    point6 = sd.get_point(x1+80, y1)
    sd.line(point5, point6, color=color, width=4)
    point7 = sd.get_point(x1+40, y1+50)
    sd.circle(point7, radius=100, color=color, width=4)
    point8 = sd.get_point(x1, y1+80)
    sd.circle(point8, radius=20, color=color, width=4)
    point9 = sd.get_point(x1+80, y1+80)
    sd.circle(point9, radius=20, color=color, width=4)
    return
...
for _ in range(10):
    ...
    smile(x1=point.x, y1=point.y, color=color1)
